...
- Words-mode fallback: `sampleNormalWords` in `TypingTest`; uses `randomSeed()` plus `mulberry32`.
- Time-mode fallback: `src/lib/localPrompt.ts`; accepts a seed but callers currently omit it. Its word bank is letters-oriented, so enabling punctuation/numbers does not itself create those characters.
